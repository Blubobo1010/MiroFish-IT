    ...
    <div class="main-split-layout">
            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">Generazione in corso: {{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Pulsante Passo Successivo - mostrato dopo il completamento -->
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Entra nell'interazione approfondita</span>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (contenuto generato, ma l'intera sezione potrebbe non essere ancora completata) -->
                  ...
                  <!-- Section Complete (generazione sezione completata) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Quando e la risposta finale, mostra un avviso speciale -->
...
const toggleRawResult = (timestamp, event) => {
  // Salva la posizione del pulsante rispetto alla viewport
  ...
  // Dopo l'aggiornamento del DOM, regola la posizione di scorrimento per mantenere il pulsante nella stessa posizione
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Icona lampadina - rappresenta l'analisi
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // Icona globo - rappresenta la ricerca panoramica
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Icona utenti - rappresenta il dialogo
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Icona fulmine - rappresenta la velocita
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // Icona grafico - rappresenta le statistiche
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Icona database - rappresenta le entita
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // Estrai la domanda di analisi
    const queryMatch = text.match(/分析问题:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Estrai lo scenario di previsione
    const reqMatch = text.match(/预测场景:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Estrai dati statistici - corrisponde al formato "fatti di previsione correlati: X elementi"
    ...
    // Estrai sotto-domande - estrazione completa, senza limiti di quantita
    ...
    // Estrai fatti chiave - estrazione completa, senza limiti di quantita
    ...
    // Estrai entita principali - estrazione completa, include riepilogo e conteggio fatti correlati
    ...
    // Estrai catene di relazioni - estrazione completa, senza limiti di quantita
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Estrai fatti attualmente validi - estrazione completa, senza limiti di quantita
    ...
    // Estrai fatti storici/scaduti - estrazione completa, senza limiti di quantita
    ...
    // Estrai entita coinvolte - estrazione completa, senza limiti di quantita
...
const parseInterview = (text) => {
  ...
  try {
    // Estrai il tema dell'intervista
    const topicMatch = text.match(/\*\*采访主题:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Estrai il numero di intervistati (es. "5 / 9 Agent simulati")
    ...
    // Estrai il motivo della selezione degli intervistati
    const reasonMatch = text.match(/### 采访对象选择理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Analizza il motivo di selezione di ogni persona
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Formato 1: numero. **nome (index=X)**: motivo
        // Esempio: 1. **alumni_345 (index=1)**: come ex-alunno...
        ...
        // Formato 2: - selezione nome (index X): motivo
        // Esempio: - selezione genitore_601 (index 0): come rappresentante dei genitori...
        if (!headerMatch) {
        ...
        // Esempio: - **genitore_601 (index 0)**: come rappresentante dei genitori...
        if (!headerMatch) {
        ...
        if (name) {
          // Salva il motivo della persona precedente
          if (currentName && currentReason.length > 0) {
            reasons[currentName] = currentReason.join(' ').trim()
          }
          // Inizia una nuova persona
          currentName = name
          currentReason = reasonStart ? [reasonStart.trim()] : []
        } else if (currentName && line.trim() && !line.match(/^未选|^综上|^最终选择/)) {
          // Riga di continuazione del motivo (esclude il paragrafo riassuntivo finale)
          currentReason.push(line.trim())
        }
      }
      
      // Salva il motivo dell'ultima persona
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Estrai ogni record di intervista
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Estrai il titolo (es. "studente", "educatore", ecc.)
      ...
      if (nameRoleMatch) {
        ...
        // Imposta il motivo di selezione della persona
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Se la prima domanda e preceduta da "1.", richiede gestione speciale
      ...
      if (answerMatch) {
        ...
        // Logica di fallback piattaforma (compatibilita formato precedente: solo un marcatore piattaforma)
        if (!twitterMatch && redditMatch) {
          ...
          // Nessun marcatore piattaforma (formato molto vecchio), usa l'intero testo come risposta
    ...
    // Estrai il riepilogo dell'intervista
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Estrai il numero di risultati
    const countMatch = text.match(/找到\s*(\d+)\s*条/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Estrai fatti correlati - estrazione completa, senza limiti di quantita
    ...
    // Prova a estrarre informazioni sugli archi (se presenti)
    ...
    // Prova a estrarre informazioni sui nodi (se presenti)
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Totale ${props.result.facts.length} elementi`)
          ]),
          ...
          }, expandedFacts.value ? `Comprimi ▲` : `Espandi tutti ${props.result.facts.length} elementi ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Totale ${props.result.entities.length} elementi`)
          ]),
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) =>
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount} el.`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `Comprimi ▲` : `Espandi tutti ${props.result.entities.length} elementi ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Totale ${props.result.relations.length} elementi`)
          ]),
          ...
          }, expandedRelations.value ? `Comprimi ▲` : `Espandi tutti ${props.result.relations.length} elementi ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sotto-domande di analisi generate dalla query a deriva'),
            h('span', { class: 'panel-count' }, `Totale ${props.result.subQueries.length} elementi`)
          ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Totale ${props.result.activeFacts.length} elementi`)
          ]),
          ...
          }, expandedActive.value ? `Comprimi ▲` : `Espandi tutti ${props.result.activeFacts.length} elementi ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Totale ${props.result.historicalFacts.length} elementi`)
          ]),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Prova a estrarre informazioni temporali [time - time]
                  (() => {
          ...
          }, expandedHistorical.value ? `Comprimi ▲` : `Espandi tutti ${props.result.historicalFacts.length} elementi ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Mantiene lo stato di selezione piattaforma indipendente per ogni coppia domanda-risposta
    ...
    // Verifica se e testo segnaposto della piattaforma
    ...
    // Prova a dividere le risposte per numero di domanda
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Se non corrisponde, fallback al formato "numero."
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Se non trovata numerazione o trovata solo una, restituisci l'intero testo
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Divisione fallita: la prima domanda restituisce la risposta completa, le altre restituiscono vuoto
      return qIdx === 0 ? answer : ''
    }
    
    // Verifica se una domanda ha risposte su doppia piattaforma (filtra testo segnaposto)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Entrambe le piattaforme hanno risposte reali (non segnaposto) e contenuti diversi
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Pulsanti commutazione doppia piattaforma (mostrati solo con risposte reali su entrambe le piattaforme)
                  ...
                  // Pulsante Espandi/Comprimi (non mostrato per testo segnaposto)
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Risultati della ricerca'),
            h('span', { class: 'panel-count' }, `Totale ${props.result.facts.length} elementi`)
          ]),
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Totale ${props.result.edges.length} elementi`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Totale ${props.result.nodes.length} elementi`)
          ]),
...
// Passo attualmente attivo (per la visualizzazione superiore)
const activeStep = computed(() => {
  ...
  // Se non c'e nessuno attivo, restituisci l'ultimo passo completato
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // Altrimenti restituisci il primo passo
  return steps[0] || { noLabel: '--', title: 'In attesa di inizio', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Rimuovi il titolo di secondo livello iniziale (## xxx), poiche il titolo della sezione e gia mostrato all'esterno
  let processedContent = content.replace(/^##\s+.+\n+/, '')
  
  // Gestisci blocchi di codice
  let html = processedContent.replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="code-block"><code>$2</code></pre>')
  
  // Gestisci codice inline
  ...
  // Gestisci blocchi citazione
  html = html.replace(/^> (.+)$/gm, '<blockquote class="md-quote">$1</blockquote>')
  
  // Gestisci liste - supporta sotto-liste
  ...
  // Avvolgi lista non ordinata
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Avvolgi lista ordinata
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Pulisci tutti gli spazi tra gli elementi della lista
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Pulisci gli spazi dopo il tag di apertura della lista
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Pulisci gli spazi prima del tag di chiusura della lista
  html = html.replace(/\s+<\/ul>/g, '</ul>')
  html = html.replace(/\s+<\/ol>/g, '</ol>')
  
  // Gestisci grassetto e corsivo
  html = html.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
  html = html.replace(/\*(.+?)\*/g, '<em>$1</em>')
  html = html.replace(/_(.+?)_/g, '<em>$1</em>')
  
  // Gestisci linee separatrici
  html = html.replace(/^---$/gm, '<hr class="md-hr">')
  
  // Gestisci a capo - righe vuote diventano separatori di paragrafo, singoli a capo diventano <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  
  // Avvolgi nei paragrafi
  ...
  // Pulisci tag <br> prima e dopo gli elementi a blocco
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Pulisci situazioni <p><br> seguite da elementi a blocco (causate da righe vuote in eccesso)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Pulisci tag <br> consecutivi
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Pulisci <br> prima dei tag di apertura paragrafo che seguono immediatamente elementi a blocco
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Correggi la numerazione delle liste ordinate non consecutive: quando singoli <ol> sono separati da paragrafi, mantieni la numerazione progressiva
...
const getLogLevelClass = (log) => {
  ...
  // INFO usa colore predefinito, non contrassegnare come success
  return ''
}
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // La logica di scorrimento viene gestita nel nextTick dopo la fine del ciclo
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Se l'attivita e completata, scorri in alto; altrimenti scorri in basso per seguire i log piu recenti
            if (isComplete.value) {
...
// Estrai il contenuto della risposta finale - estrai il contenuto della sezione dalla risposta LLM
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Prova a estrarre il contenuto dal tag <final_answer>
  ...
  // Prova a trovare il contenuto dopo Final Answer: (supporta vari formati)
  ...
  // Se il contenuto e lungo e contiene formato markdown, prova a rimuovere il processo di pensiero e restituisci
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Rimuovi il processo di pensiero che inizia con Thought:
